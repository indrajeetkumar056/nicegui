...
from configs import OHLC_DB_NAME_PROD, OHLC_TABLE_MAPPING, OHLC_DB_NAME_DEV
    ...
    def process_fetched_data(self, products, start_datetime, end_datetime):
        ...
        try:
            ...
            if any(t in timeframes for t in ['1', '5', '60']):
                final_dict = process_tas_chunk_df(self.db, processed_df, timeframes, start_datetime, end_datetime)
                Datastore.processed_dict[sink_name] = final_dict
            if "1440" in timeframes:
                final_dict = process_tas_daily_chunk_df(self.db, processed_df, '1D', products, start_datetime, end_datetime)
