...
def calculate_vap_by_day(df, from_date, to_date):
    ...
    vap_df = df.groupby(["Instrument", "Price"]).agg(
    ...
    vap_df["vap_entry"] = vap_df.apply(lambda row: f"{row['Price']}:{int(row['buyVolume'])}-{int(row['sellVolume'])}", axis=1)

    # Group VAP entries per Instrument
    vap_final = vap_df.groupby("Instrument")["vap_entry"].apply(lambda x: ";".join(x)).reset_index()
    vap_final.rename(columns={"vap_entry": "vap", "Instrument": "product"}, inplace=True)
    # Set 'time' value
    time_value = adjusted_from_date if from_date.strftime("%H:%M:%S") == "00:00:00" else adjusted_to_date
    vap_final["time"] = time_value

    return vap_final

def calculate_vap(df, timeframe, products, start_datetime, end_datetime):
    ...
    for i in range(num_days):
        ...
        try:
            for s_time, prod in prod_stimes_map.items():
                ...
                # Calculate OHLCV
                ohlcv_df = calculate_vap_by_day(df_filtered, from_date, to_date)
...
def process_chunk(df, timeframe, products, start_datetime, end_datetime):
    ohlcv_by_timeframe = calculate_vap(df, timeframe, products, start_datetime, end_datetime)
    print(ohlcv_by_timeframe)
    return ohlcv_by_timeframe

def process_vap_daily_chunk_async(uuid, df, timeframe, products, start_datetime, end_datetime):
